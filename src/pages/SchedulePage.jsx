...
import React, { useState } from 'react'; import { useNavigate, Link } from 'react-router-dom'; import emailjs from 'emailjs-com'; import { useCart } from '../context/CartContext'; import { calculatePrice } from '../utils/pricing'; import { FaChevronDown } from 'react-icons/fa';

const generatePresetDates = () => { const presets = []; for (let i = 0; i < 6; i++) { const date = new Date(); date.setDate(date.getDate() + i); const label = i === 0 ? 'Today' : i === 1 ? 'Tomorrow' : i === 2 ? 'Day After' : date.toLocaleDateString('en-US', { weekday: 'long' }); const dateFormatted = date.toLocaleDateString('en-US', { weekday: i < 3 ? 'long' : undefined, month: '2-digit', day: '2-digit', year: 'numeric' }); const isoDate = date.toISOString().split('T')[0]; presets.push({ label, dateFormatted, value: isoDate }); } return presets; };

const presetDates = generatePresetDates();

function SchedulePage() { const { cart } = useCart(); const { finalPrice } = calculatePrice(cart); const [formData, setFormData] = useState({ name: '', email: '', phone: '', address: '', date: '', time: '', customDateFormatted: '' }); const navigate = useNavigate();
